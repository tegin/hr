odoo.define("web.FieldEvaluationSkill", function(require) {
    "use strict";
    ...
    var AbstractGroupedOne2ManyRenderer = ListRenderer.extend({
        ...
        _renderRow: function(record, isLast) {
            return $(
                qweb.render(this.dataRowTemplate, {
                    id: record.id,
        ...
        _renderBody: function() {
            ...
            for (var key in grouped_by) {
                ...
                if (self.addCreateLine) {
                    $title_row
                        .find(".o_group_name")
                        .append(self._renderAddItemButton(group));
                }
            }
    ...
    var EvalutionSkillsRenderer = AbstractGroupedOne2ManyRenderer.extend({
        groupBy: "skill_type_id",
        dataRowTemplate: "hr_evaluation_skill_data_row",
        addLineButtonTemplate: "hr_evaluation_skill_group_add_item",
        ...
        _getCreateLineContext: function(group) {
            ...
            return group
                ? _.extend(
                      {default_skill_type_id: group[0].data[this.groupBy].data.id},
                      ctx
                  )
                : ctx;
        },
    ...
    var FieldEvaliationSkills = FieldOne2Many.extend({
        /**
         * @override
         * @private
         */
        _getRenderer: function() {
            return EvalutionSkillsRenderer;
        },
    });

    field_registry.add("hr_evaluation_skills", FieldEvaliationSkills);

    return {
        FieldEvaliationSkills: FieldEvaliationSkills,
        EvalutionSkillsRenderer: EvalutionSkillsRenderer,
    };
});
